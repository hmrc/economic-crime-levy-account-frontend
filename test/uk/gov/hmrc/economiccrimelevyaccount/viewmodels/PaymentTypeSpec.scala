...
class PaymentTypeSpec extends SpecBase {
  ...
  "reads" should {
    "return the payments type deserialized from it's JSON representation" in forAll { (paymentType: PaymentType) =>
  ...
  "writes" should {
    "return the serialized String from it's payment type representation" in forAll { (paymentType: PaymentType) =>
      val json = Json.toJson(paymentType)

      json shouldBe JsString(paymentType.toString)
